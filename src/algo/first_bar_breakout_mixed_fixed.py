"""
First Bar Breakout Strategy - Mixed Approach: First Bar SL + Fixed Dollar TP

This variant uses:
- Stop Loss: First bar high (for short) or low (for long)
- Take Profit: Entry ± fixed dollar amount

- First bar: 9:30 AM ET (first 1-minute bar)
- Entry: Price breaks above first bar high (long) or below first bar low (short)
- Stop Loss: First bar low (long) or first bar high (short)
- Take Profit: Entry ± fixed_tp_amount (in dollars)
...
class TradeResult:
    ...
    stop_loss: float
    take_profit: float
    fixed_tp_amount: float  # The fixed dollar TP amount used
...
class FirstBarBreakoutMixedFixedStrategy:
    """
    First Bar Breakout Strategy with Mixed Approach: First Bar SL + Fixed Dollar TP.

    Uses first bar high/low for stop-loss and fixed dollar amount for take-profit.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            fixed_tp_amount: Fixed dollar amount for TP (default: 1.0)
        """
        self.first_bar_time = first_bar_time
        self.market_open = market_open
        self.market_close = market_close
        self.fixed_tp_amount = fixed_tp_amount
    ...
    def backtest(self, df: pl.DataFrame) -> list[TradeResult]:
        ...
        """
        logger.info("=" * 80)
        logger.info("FIRST BAR BREAKOUT STRATEGY (MIXED: FIRST BAR SL + FIXED TP)")
        logger.info(f"Fixed TP Amount: ${self.fixed_tp_amount:.2f}")
        logger.info("=" * 80)
        ...
        for row in first_bars.iter_rows(named=True):
            ...
            # Set stop-loss at first bar high/low and TP at fixed amount
            if direction == "long":
                stop_loss = first_bar_low
                take_profit = entry_price + self.fixed_tp_amount
            else:  # short
            ...
            trade = TradeResult(
                ...
                fixed_tp_amount=self.fixed_tp_amount,
    ...
    def generate_summary_stats(self, results: list[TradeResult]) -> dict:
        ...
        # SL distance statistics
        avg_sl_distance = (
            sum(r.sl_distance for r in results) / total_trades
            if total_trades > 0
            else 0
        )

        stats = {
            "fixed_tp_amount": results[0].fixed_tp_amount if results else 0,
    ...
    def print_summary(self, stats: dict) -> None:
        ...
        logger.info("=" * 80)
        logger.info(f"Fixed TP Amount: ${stats['fixed_tp_amount']:.2f}")
        logger.info(f"Average SL Distance: ${stats['avg_sl_distance']:.2f}")
...
def export_results_to_csv(results: list[TradeResult], output_path: str) -> None:
    ...
    data = {
        ...
        "fixed_tp_amount": [r.fixed_tp_amount for r in results],
        "sl_distance": [r.sl_distance for r in results],
