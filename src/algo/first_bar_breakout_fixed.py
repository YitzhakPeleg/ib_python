"""
First Bar Breakout Strategy - Fixed Dollar TP/SL Variant

This variant uses fixed dollar amounts for take-profit and stop-loss
instead of multiples of the first bar range.

- First bar: 9:30 AM ET (first 1-minute bar)
- Entry: Price breaks above first bar high (long) or below first bar low (short)
- Stop Loss: Entry ± fixed_sl_amount (in dollars)
...
class TradeResult:
    ...
    stop_loss: float
    take_profit: float
    fixed_amount: float  # The fixed dollar amount used
...
class FirstBarBreakoutFixedStrategy:
    """
    First Bar Breakout Strategy with Fixed Dollar TP/SL.

    Uses fixed dollar amounts for take-profit and stop-loss instead of
    multiples of the first bar range.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            fixed_amount: Fixed dollar amount for both TP and SL (default: 1.0)
        """
    ...
    def backtest(self, df: pl.DataFrame) -> list[TradeResult]:
        ...
        """
        logger.info("=" * 80)
        logger.info("FIRST BAR BREAKOUT STRATEGY (FIXED TP/SL) - BACKTEST")
        logger.info(f"Fixed Amount: ${self.fixed_amount:.2f}")
        logger.info("=" * 80)
        ...
        for row in first_bars.iter_rows(named=True):
            ...
            # Set stop-loss and take-profit with fixed dollar amounts
            if direction == "long":
                stop_loss = entry_price - self.fixed_amount
                take_profit = entry_price + self.fixed_amount
            else:  # short
                stop_loss = entry_price + self.fixed_amount
                take_profit = entry_price - self.fixed_amount
    ...
    def generate_summary_stats(self, results: list[TradeResult]) -> dict:
        ...
        stats = {
            "fixed_amount": results[0].fixed_amount if results else 0,
    ...
    def print_summary(self, stats: dict) -> None:
        ...
        logger.info("=" * 80)
        logger.info(f"Fixed Amount: ${stats['fixed_amount']:.2f}")
...
def export_results_to_csv(results: list[TradeResult], output_path: str) -> None:
    ...
    data = {
        ...
        "fixed_amount": [r.fixed_amount for r in results],
