"""Fetch SPY 1-min bars for 2020-01-01 through 2021-12-31 as a separate file.

Existing SPY_1_min.parquet starts 2022-01-06 — this fills the two years
before that into its own file (SPY_2020_2021_1_min.parquet) rather than
merging, so it can be used as an independent out-of-sample check without
touching the data the current strategy results are based on.

Same chunked-backward pattern as fetch_spy_history.py: IB caps how much
1-min data one request can return, so walk backward in fixed-size chunks
until IB returns zero bars or an error, or we reach the 2020-01-01 target.
...
class IBSettings(BaseSettings):
    ...
    ib_port: int = 4002
    ib_client_id: int = 126  # distinct from other fetch scripts' client ids


settings = IBSettings()

TICKER = "SPY"
OUT_TICKER_LABEL = "SPY_2020_2021"  # separate file, not merged into SPY_1_min.parquet
TARGET_START = datetime(2020, 1, 1)
CHUNK = timedelta(days=30)
SLEEP_BETWEEN_REQUESTS = 2.0
MAX_CHUNKS = 30  # ~2 years / 30-day chunks + margin

# Start just before the existing file's earliest bar (2022-01-06 09:30 ET).
end_date = datetime(2022, 1, 6, 9, 29)
...
if not all_new_frames:
    ...
    new_data = new_data.filter(pl.col("DateTime") >= pl.lit(TARGET_START).dt.replace_time_zone("US/Eastern"))
